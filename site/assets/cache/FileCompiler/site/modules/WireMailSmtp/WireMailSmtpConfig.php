...
class WireMailSmtpConfig extends \ProcessWire\Wire {
    ...
    public function getConfig(array $data) {
        ...
        $siteconfig = is_array(\ProcessWire\wire('config')->wiremailsmtp) ? \ProcessWire\wire('config')->wiremailsmtp : array();
...
//            $fieldset->add($field);
        ...
        $field->icon = 'filter';
        $field->columnWidth = 100;
        $field->collapsed = \ProcessWire\Inputfield::collapsedNo;
...
//                }
        ...
        if(\ProcessWire\wire('session')->test_settings) {
            ...
            $post = \ProcessWire\wire('input')->post;
