...
class ProcessAdminActions extends \ProcessWire\Process implements \ProcessWire\Module, \ProcessWire\ConfigurableModule {
 ...
 require_once(\ProcessWire\wire('files')->compile($this->getActionPath($this->wire('input')->get->action,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false))));
    ...
    public function ___executeRun() {
        ...
        if($numGetParams > 1 && $numGetParams === $i) {
            $urlFields = new \ProcessWire\WireInputData();
    ...
    private function buildSelectForm() {
        ...
        $tabsWrapper = new \ProcessWire\InputfieldWrapper();
        ...
        elseif($this->wire('input')->get->installed) {
            $this->wire()->warning($this->wire('input')->get->installed . " new " . \ProcessWire\_n('action was', 'actions were', $this->wire('input')->get->installed) . " installed. If you need to adjust role access and menu status, visit please visit the <a href='".$this->wire('config')->urls->admin . 'module/edit?name=' . $this . "'>module config settings</a>.", \ProcessWire\Notice::allowMarkup);
        }
        ...
        foreach($this->data as $actionType => $actionsList) {
            ...
            $table->headerRow(array(
                ...
                \ProcessWire\__('Description'),
                \ProcessWire\__('Notes')
            ));
        ...
        if(file_exists($this->adminActionsCacheDir.$this->dbBackupFilename) && ($this->wire('user')->isSuperuser() || $this->wire('user')->hasPermission('admin-actions-restore'))) {
            ...
            if($this->wire('input')->post->restoreConfirm) {
                $backup = new \ProcessWire\WireDatabaseBackup($this->adminActionsCacheDir);
     ...
     *
     */
    private function processSelectForm(\ProcessWire\InputfieldForm $form) {
    ...
    private function buildOptionsForm() {
        ...
        if($this->dbBackup == 'optional') {
            ...
            $f->name = 'dbBackup';
            $f->label = \ProcessWire\__('Database Backup');
            $f->description = \ProcessWire\__('Do you want to automatically backup the database before executing this action?');
            $f->notes = \ProcessWire\__('This is highly recommended!');
            $form->add($f);
        }

        $f = $this->wire('modules')->get("InputfieldSubmit");
        $f->name = 'submit';
        $f->value = $this->action->executeButtonLabel ?: \ProcessWire\__('Execute Action');
        $form->add($f);
    ...
    private function getActionPath($className) {
        if(file_exists($actionPath = \ProcessWire\wire("config")->paths->root . 'site/modules/ProcessAdminActions' . '/actions/'.$className.'.action.php')) {
            return $actionPath;
        }
        elseif(file_exists($actionPath = \ProcessWire\wire("config")->paths->root . 'site/modules/ProcessAdminActions' . '/actions/'.$className.'/'.$className.'.action.php')) {
            return $actionPath;
        }
    ...
    private function getAllActions($instantiate = true) {
        ...
        $this->getActions(\ProcessWire\wire("config")->paths->root . 'site/modules/ProcessAdminActions' . '/actions/', $instantiate);
    }
    ...
    private function getActions($folderPath, $instantiate = true) {
        foreach(new \RecursiveIteratorIterator(new \RecursiveDirectoryIterator($folderPath, \RecursiveDirectoryIterator::SKIP_DOTS), \RecursiveIteratorIterator::SELF_FIRST) as $file) {
            ...
            if($file->isFile()) {
                ...
                $className = $matches[1];
                if($instantiate) {
 require_once(\ProcessWire\wire('files')->compile($this->getActionPath($className,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false))));
    ...
    protected function modifyBreadcrumb($event) {
        $this->wire('breadcrumbs')->add(new \ProcessWire\Breadcrumb('./options?action='.$this->wire('input')->get->action, $this->getActionTitle($this->wire('input')->get->action, $this->data[$this->actionTypes[$this->wire('input')->get->action]][$this->wire('input')->get->action])));
    }
    ...
    private function backupDb() {
        if(!file_exists($this->adminActionsCacheDir)) \ProcessWire\wiremkdir($this->adminActionsCacheDir);
        ...
        if(!file_put_contents($this->adminActionsCacheDir . 'restoredb.php', $restoreDbCode, LOCK_EX)) throw new \ProcessWire\WireException("Unable to write file: " . $this->adminActionsCacheDir . 'restoredb.php');
    }
 ...
 require_once(\ProcessWire\wire('files')->compile($actionFilePath,array('includes'=>true,'namespace'=>true,'modules'=>false,'skipIfNamespace'=>false)));
    ...
    public function getModuleConfigInputfields(array $data) {
        ...
        $f->name = 'dbBackup';
        $f->label = \ProcessWire\__('Database Backup');
        $f->description = \ProcessWire\__('Do you want to backup the database before each action is executed?');
        $f->notes = \ProcessWire\__('You can override this setting in your custom actions by setting: protected $dbBackup = "automatic", "optional", or "disabled".');
        $f->required = true;
        ...
        $f->label = \ProcessWire\__('Show Action Code');
        $f->description = \ProcessWire\__('Show the code for the executeAction() function for superusers.');
        $f->columnWidth = 50;
        ...
        $fieldset->label = \ProcessWire\__('Action settings');
        ...
        foreach($this->actions as $actionType => $actionsList) {
            ...
            $table->headerRow(array(
                \ProcessWire\__('Title'),
                \ProcessWire\__('Info'),
                \ProcessWire\__('Roles'),
                sprintf(\ProcessWire\__('In%sMenu', \ProcessWire\wire("config")->paths->root . 'site/modules/ProcessAdminActions/ProcessAdminActions.module.php'), '&nbsp;'),
                \ProcessWire\__('Author')
            ));
            ...
            foreach($actionsList as $action => $info) {
                ...
                $rolesWrapper = new \ProcessWire\InputfieldWrapper();
                $fr = $this->wire('modules')->get("InputfieldAsmSelect");
                $fr->attr('name', 'roles_'.$info['name']);
                $fr->skipLabel = \ProcessWire\Inputfield::skipLabelHeader;
                ...
                $menuWrapper = new \ProcessWire\InputfieldWrapper();
                $fm = $this->wire('modules')->get("InputfieldCheckbox");
                $fm->attr('name', 'menu_'.$info['name']);
                $fm->skipLabel = \ProcessWire\Inputfield::skipLabelHeader;
                $fm->label = ' ';
