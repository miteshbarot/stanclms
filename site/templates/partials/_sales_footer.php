...
$(function(){
	...
	$("#app_status_process_bb").change(function(){
		var value = $(this).children("option:selected").attr('value');
	...
	$(".update_form").click(function(){
			...
			var data = $("#pl_salse_application_form").serialize();
			console.log(data); 
			$.ajax({
				...
				success:function(response){
					...
					if(response.status=="1"){
						alert('Status : APPROVE \n AIP REF NUMBER : '+response.aip_ref_number);
						return false;
					}else{
						alert("Error while updating pleaase try again!!"+response.api_msg); return false;
					}
		...
		if(valid==true){
			...
			var data = $("#cc_salse_application_form").serialize();
			console.log(data); 
			$.ajax({
	  	        url: root+"apis/form-api/",
				type: 'POST',
				...
				success:function(response){
					...
					alert('Status : APPROVE \n AIP REF NUMBER : '+response.aip_ref_number);
						...
						alert("Error while updating #"+response.api_msg); return false;
					}
				},
				error:function(error){
					console.log(error);
